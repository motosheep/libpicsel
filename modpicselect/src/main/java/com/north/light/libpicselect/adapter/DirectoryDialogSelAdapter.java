 ...
 * data 2019/12/8
 */
public class DirectoryDialogSelAdapter extends RecyclerView.Adapter<DirectoryDialogSelAdapter.PicHolder> {
    private static final String TAG = DirectoryDialogSelAdapter.class.getName();
    private Context mContext;
    private List<DirecotryBean> mResult = new ArrayList<>();
    private BindImageViewListener mBindListener;
    private OnClickListener mOnClick;
    private long mScreenWidth;

    public DirectoryDialogSelAdapter(Context mContext) {
        this.mContext = mContext;
        mScreenWidth = PicScreenUtils.getScreenWidth(mContext);
    }

    public void setData(List<DirecotryBean> data) {
        mResult.clear();
        if (data == null) {
            data = new ArrayList<>();
        }
