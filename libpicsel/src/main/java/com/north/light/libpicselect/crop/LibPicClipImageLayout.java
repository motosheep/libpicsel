 ...
 * 剪切视图
 */
public class LibPicClipImageLayout extends RelativeLayout {

    private LibPicClipZoomImageView mZoomImageView;
    private LibPicClipImageBorderView mClipImageView;
    /**
...
//    private int mVerticalPadding = (getHeight() - (getWidth() - 2 * mHorizontalPadding)) / 2;
    ...
    public LibPicClipImageLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        mZoomViewRoot = new RelativeLayout(context);
        mClipImageView = new LibPicClipImageBorderView(context);
...
//    }
    ...
    public void setImageDrawable(String path) {
        mLocalPicPath = path;
        //显示本地图片
        LibPicClipBitmapUtils bitmapUtils = new LibPicClipBitmapUtils();
    ...
    private void initZoomView() {
        try {
            if (mZoomViewRoot != null) {
                ...
                mZoomImageView = new LibPicClipZoomImageView(getContext());
                android.view.ViewGroup.LayoutParams lp = new LayoutParams(
                        android.view.ViewGroup.LayoutParams.MATCH_PARENT,
                        android.view.ViewGroup.LayoutParams.MATCH_PARENT);
